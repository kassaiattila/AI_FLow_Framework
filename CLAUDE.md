...
**v1.4.10 Sprint N** — **DONE 2026-04-28**, tag `v1.4.10` queued post-merge. LLM cost guardrail + per-tenant budget on `feature/v1.4.10-cost-guardrail-budget` (cut from `feature/v1.4.9-vault-langfuse` tip `d3e2d4a` while Sprint M PR #17 OPEN, MERGEABLE). HEAD `751107d` (S123 shipped). PR opened against `main`, queued behind Sprint M PR #17 — rebase after #17 merges. Flag defaults: `AIFLOW_COST_GUARDRAIL__ENABLED=false` / `__DRY_RUN=true` (ship-off). See `docs/sprint_n_pr_description.md` + `docs/sprint_n_retro.md`. Predecessor: **v1.4.9 Sprint M** — **DONE 2026-04-25** Vault hvac + self-hosted Langfuse on `feature/v1.4.9-vault-langfuse` (cut from `main` @ `ab63c93`). HEAD `1f02d00` (S118). PR queued against `main`, tag `v1.4.9` queued post-merge. Predecessors: **v1.4.8 Sprint L** Monitoring + Cost Enforcement (MERGED 2026-04-23, PR #16, squash `ab63c93`, tag `v1.4.8`); **v1.4.5 Sprint J** UC2 RAG (queued); **v1.4.3** Phase 1d (MERGED 2026-04-24, PR #9, tag `v1.4.3-phase-1d`). | API: 8102 | UI: 5173 | Vault dev: 8210 | Langfuse dev: 3000 | Langfuse Postgres: 5434
