...
const EMAIL_PIPELINE = ["parse", "attachments", "classify", "entities", "priority", "routing"];
...
export default function EmailIntentProcessorPage() {
  ...
  const loadData = useCallback(() => {
    ...
    fetch("/api/emails")
      ...
      .then((data: { emails: EmailProcessingResult[]; source?: string }) => {
        ...
        if (data.emails.length > 0 && !selected) setSelected(data.emails[0]);
      })
  ...
  const handleProcess = useCallback(async () => {
    ...
    for (const file of uploadedFiles) {
      try {
        ...
        if (res.ok) { const data = await res.json(); if (data.source) setSource(data.source as "backend" | "subprocess"); }
      } catch { /* continue */ }
    }
    setUploadedFiles([]);
    setProcessing(false);
  ...
  const intentOptions = useMemo(() => [...new Set(emails.map((e) => e.intent?.intent_id).filter(Boolean))] as string[], [emails]);
  const priorityOptions = useMemo(() => ([...new Set(emails.map((e) => e.priority?.priority_level).filter(Boolean))].sort()) as number[], [emails]);
  ...
  const avgConfidence = totalEmails > 0 ? emails.reduce((s, e) => s + (e.intent?.confidence || 0), 0) / totalEmails : 0;
  ...
  // Pipeline steps from selected email
  const pipelineSteps = EMAIL_PIPELINE.map((name) => ({
    name,
    status: (selected?.intent ? "completed" : "pending") as "completed" | "pending",
  }));
  ...
  const exportButton = (
    <ExportButton
      ...
      rows={filtered.map((e) => [e.sender, e.subject, e.intent?.intent_display_name || "", String(e.intent?.confidence || 0), String(e.priority?.priority_level || ""), e.routing?.department_name || "", e.received_date])}
    />
  );
  ...
  return (
    ...
    >
      {/* 1. Input — upload + process */}
      <div className="flex items-center gap-3">
        <div className="flex-1"><EmailUploadZone onFilesUploaded={handleFilesUploaded} /></div>
        {uploadedFiles.length > 0 && (
          <button onClick={handleProcess} disabled={processing} className="px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium disabled:opacity-50">
      ...
      <PipelineBar steps={pipelineSteps} source={source} isProcessing={processing} />
        ...
        <div>
          {selected ? (
            <Tabs defaultValue="preview">
              ...
              <TabsContent value="preview" className="mt-4"><EmailPreview email={selected} highlightedEntity={highlightedEntity} /></TabsContent>
              <TabsContent value="intent" className="mt-4">{selected.intent ? <IntentBadgeDetail intent={selected.intent} /> : <p className="text-muted-foreground text-sm">{t("email.noIntent")}</p>}</TabsContent>
              <TabsContent value="entities" className="mt-4">{selected.entities ? <EntityList entities={selected.entities} highlightedEntity={highlightedEntity} onEntityHover={setHighlightedEntity} /> : <p className="text-muted-foreground text-sm">{t("email.noEntity")}</p>}</TabsContent>
              <TabsContent value="routing" className="mt-4">{selected.routing && selected.priority ? <RoutingCard routing={selected.routing} priority={selected.priority} /> : <p className="text-muted-foreground text-sm">{t("email.noRouting")}</p>}</TabsContent>
            </Tabs>
          ) : (
