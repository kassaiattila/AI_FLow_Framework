  ...
  const loadData = useCallback(() => {
    ...
    fetch("/api/cubix")
      ...
      .then((data: { courses: CubixCourseResult[]; source?: string }) => {
        ...
        if (data.courses.length > 0 && !selected) setSelected(data.courses[0]);
      })
    ...
    >
      ...
      {courses.length > 1 && (
        <div className="flex gap-2 overflow-x-auto pb-1">
          {courses.map((c) => (
            <button key={c.course_id} onClick={() => { setSelected(c); setSelectedSlug(null); }}
              className={`px-3 py-1 rounded-full text-xs whitespace-nowrap border transition-colors ${selected?.course_id === c.course_id ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>
              {c.course_title}
            </button>
          ))}
        </div>
          ...
          <TabsContent value="pipeline" className="mt-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              {selectedFile ? <FileDetail file={selectedFile} /> : (
                <Card><CardContent className="py-12 text-center text-muted-foreground text-sm">{t("cubix.selectFile")}</CardContent></Card>
              )}
            </div>
          </TabsContent>
