        ...
        (data) => {
          try {
            ...
            if (msg.event === "complete") {
              ...
              setResult(
                `${ok}/${files.length} ${translate("aiflow.documentUpload.files")} processed`,
              );
              setProcessing(false);
              setFileProgress((prev) =>
                prev.map((fp) => ({
            ...
            <div className="flex gap-2">
              ...
              >
                {uploading
                  ...
                  : translate("aiflow.documentUpload.process")}
              </button>
            </div>
          </div>
          {files.map((f, i) => (
  ...
  return [
    {
      ...
      render: (item) => (
        <span
          className="block max-w-[200px] truncate font-medium text-gray-900 dark:text-gray-100"
          title={fileName(String(item.source_file ?? ""))}
        >
    ...
    },
    {
      ...
      render: (item) => (
        <span
          className="block max-w-[160px] truncate text-gray-600 dark:text-gray-400"
          title={(item.vendor as DocVendor | null)?.name ?? ""}
        >
    ...
    },
    {
      ...
      render: (item) => {
        ...
        return (
          <span
            className={`rounded-full px-2 py-0.5 text-xs font-medium ${colors[level]}`}
          >
            {Math.round(conf * 100)}%
          </span>
        );
...
export function Documents() {
  ...
  const { data: configsData } = useApi<ConfigsResponse>(
    "/api/v1/documents/extractor/configs",
  );
  ...
  const processed = docs.filter(
    (d) => d.extraction_confidence && d.extraction_confidence > 0,
  ).length;
  ...
  const approved = docs.filter(
    (d) =>
      (d as unknown as Record<string, unknown>).status === "approved" ||
      d.validation?.is_valid === true,
  ).length;
  const rejected = docs.filter(
    (d) => (d as unknown as Record<string, unknown>).status === "rejected",
  ).length;
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const ids = selectedDocs
        .map((d) => (d as unknown as DocItem).id)
        .filter(Boolean);
      await fetchApi<{ deleted: number }>(
        "POST",
        "/api/v1/documents/delete-bulk",
        { ids },
      );
      ...
      actions={
        <div className="flex items-center gap-2">
          {configsData && configsData.configs.length > 0 && (
            ...
            >
              <option value="all">
                {translate("aiflow.documents.filterAll")}
              </option>
      ...
      {tab === "upload" ? (
        ...
        <>
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-xs font-medium text-gray-500">
                {translate("aiflow.documents.filterProcessed")}
              </p>
              <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">
                {processed}
              </p>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-xs font-medium text-gray-500">
                {translate("aiflow.documents.unprocessed")}
              </p>
          ...
          {error ? (
            ...
            <DataTable
              ...
              onRowClick={(item) => {
                const docId = (item as unknown as DocItem).id;
                if (docId)
                  navigate(`/documents/${encodeURIComponent(docId)}/show`);
              }}
            />
          )}
        </>
