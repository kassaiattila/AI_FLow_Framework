...
interface SpanMetricsResponse {
  window_h: number;
  from_ts: string;
  to_ts: string;
  total_spans: number;
  models: ModelSpanMetric[];
  source: string;
}
...
export function Monitoring() {
  ...
  const {
    ...
    error: he,
    refetch,
  } = useApi<HealthResponse>("/api/v1/admin/health");
  const { data: metrics } = useApi<MetricsResponse>("/api/v1/admin/metrics");
  const { data: spanMetrics, error: spanError } = useApi<SpanMetricsResponse>(
    "/api/v1/monitoring/span-metrics?window_h=24",
  );
  ...
  async function handleRestart() {
    ...
    try {
      await fetchApi<unknown>(
        "POST",
        `/api/v1/admin/services/${restartTarget}/restart`,
      );
  ...
  const getMetric = (name: string) =>
    metrics?.metrics.find((m) => m.service_name === name);
  const healthy =
    health?.services.filter((s) => s.status === "healthy").length ?? 0;

  return (
    <PageLayout
      titleKey="aiflow.monitoring.title"
      subtitleKey="aiflow.monitoring.subtitle"
      ...
      actions={
        <div className="flex items-center gap-2">
          <select
            value={autoRefresh ?? ""}
            onChange={(e) =>
              setAutoRefresh(e.target.value ? Number(e.target.value) : null)
            }
          ...
          >
            <option value="">
              {translate("aiflow.monitoring.autoRefresh")}: Off
            </option>
          ...
          <button
            onClick={refetch}
            className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400"
          >
            {translate("aiflow.monitoring.refresh")}
          </button>
        </div>
      }
    >
      {hl ? (
        ...
        <ErrorState error={he} onRetry={refetch} />
      ) : health ? (
        <>
          {/* Status banner */}
          <div
            className={`mb-4 rounded-lg p-3 text-sm font-medium ${health.overall_status === "healthy" ? "bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400" : "bg-amber-50 text-amber-700"}`}
          >
            {health.overall_status === "healthy"
              ? translate("aiflow.monitoring.allOperational")
              : translate("aiflow.monitoring.degradedBanner")}{" "}
            — {healthy}/{health.total}{" "}
          ...
          <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              ...
              <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">
                {Math.round(
                  health.services.reduce((s, svc) => s + svc.latency_ms, 0) /
                    health.total,
                )}
                ms
              </p>
          ...
          {spanError ? (
            <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
              Span metrics unavailable:{" "}
              <span className="font-mono">{spanError}</span>
            </div>
          ) : spanMetrics ? (
                    ...
                    spanMetrics.models.map((m) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 tabular-nums text-gray-500">
                          {m.avg_duration_ms.toFixed(0)}
                        </td>
                        <td className="px-4 py-2 tabular-nums text-gray-500">
                          {m.p95_duration_ms.toFixed(0)}
                        </td>
                        <td className="px-4 py-2 tabular-nums text-xs text-gray-500">
            ...
            {health.services.map((svc) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-xl border bg-white p-3 dark:bg-gray-900 ${color}`}
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-sm ${svc.status === "healthy" ? "text-green-600" : svc.status === "degraded" ? "text-amber-600" : "text-red-600"}`}
                    >
                      {icon}
                    </span>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    ...
                    {m
                      ? ` · p95: ${Math.round(m.p95_latency_ms)}ms · ${m.success_rate}%`
                      : ""}
                  </div>
                  <button
      ...
      <ConfirmDialog
        open={!!restartTarget}
        title={translate("aiflow.monitoring.restartConfirm").replace(
          "{{service}}",
          restartTarget ?? "",
        )}
