...
export function Runs() {
  ...
  const { data, loading, error, refetch } =
    useApi<RunsResponse>("/api/v1/runs");
  ...
  const columns: Column<Record<string, unknown>>[] = [
    {
      ...
      render: (item) => (
        <span className="font-mono text-xs text-gray-600 dark:text-gray-400">
          {String(item.run_id).substring(0, 8)}...
        </span>
      ),
    },
    {
      key: "skill_name",
      label: translate("aiflow.runs.skill"),
      render: (item) => (
        <span className="font-medium text-gray-900 dark:text-gray-100">
          {String(item.skill_name ?? item.workflow_name)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      getValue: (item) => (item.total_duration_ms as number) ?? 0,
      render: (item) => (
    ...
    },
    {
      ...
      getValue: (item) => (item.total_cost_usd as number) ?? 0,
      render: (item) => (
        <span className="text-gray-500">
          {(item.total_cost_usd as number) > 0
            ? `$${(item.total_cost_usd as number).toFixed(3)}`
            : "—"}
        </span>
      ),
    },
    ...
    },
    {
      ...
      sortable: false,
      render: (item) => {
        const traceId = item.trace_id as string | undefined | null;
        if (!traceId) return <span className="text-xs text-gray-400">—</span>;
        return (
          <button
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/runs/${item.run_id as string}#trace`);
            }}
            className="rounded-md border border-brand-300 px-2 py-0.5 text-xs font-medium text-brand-600 hover:bg-brand-50 dark:border-brand-700 dark:text-brand-400 dark:hover:bg-brand-900/20"
            title="Open trace"
          >
            ↗ trace
          </button>
    ...
    <PageLayout titleKey="aiflow.runs.title">
      {error ? (
        ...
        <DataTable
          ...
          columns={columns}
          loading={loading}
          searchKeys={["skill_name", "workflow_name", "status"]}
