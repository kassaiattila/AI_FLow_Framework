...
export function Admin() {
  ...
  const {
    ...
    error: ue,
    refetch: ur,
  } = useApi<UsersResponse>("/api/v1/admin/users");
  const {
    data: keysData,
    loading: kl,
    error: ke,
    refetch: kr,
  } = useApi<KeysResponse>("/api/v1/admin/api-keys");
  ...
  const userCols: Column<Record<string, unknown>>[] = [
    ...
    {
      key: "is_active",
      label: translate("aiflow.admin.status"),
      render: (item) => {
        ...
        return (
          ...
          >
            {active
              ? translate("aiflow.admin.active")
              : translate("aiflow.admin.inactive")}
          </span>
        );
      },
    },
    {
      key: "created_at",
      label: translate("aiflow.admin.created"),
      render: (item) => (
  ...
  const keyCols: Column<Record<string, unknown>>[] = [
    ...
    {
      key: "prefix",
      label: translate("aiflow.admin.prefix"),
      render: (item) => (
      ...
      render: (item) => {
        ...
        return (
          <button
            onClick={(e) => {
              e.stopPropagation();
              setRevokeTarget(item.id as string);
            }}
  ...
  return (
    <PageLayout
      titleKey="aiflow.admin.title"
      subtitleKey="aiflow.admin.subtitle"
      actions={
        <button
          data-testid={
            tab === "users" ? "admin-create-user" : "admin-generate-key"
          }
    ...
    >
      <div className="mb-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex gap-6">
          {(["users", "keys"] as const).map((t) => (
            ...
            >
              {t === "users"
                ? translate("aiflow.admin.users")
                : translate("aiflow.admin.apiKeys")}
            </button>
          ))}
        </div>
      </div>
      ...
      {tab === "users" ? (
        ue ? (
          <ErrorState error={ue} onRetry={ur} />
        ) : (
          <DataTable
            data={
              (usersData?.users ?? []) as unknown as Record<string, unknown>[]
            }
            columns={userCols}
            loading={ul}
        ...
        <ErrorState error={ke} onRetry={kr} />
      ) : (
        <DataTable
          data={(keysData?.keys ?? []) as unknown as Record<string, unknown>[]}
          columns={keyCols}
          loading={kl}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-700 dark:bg-gray-900">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">
                  {translate("aiflow.admin.userName")}
                </label>
                <input
                  type="text"
                  value={newName}
            ...
            {revealedKey ? (
              <div className="space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  >
                    {keyCopied
                      ? translate("aiflow.admin.copied")
                      : translate("aiflow.admin.copyKey")}
                  </button>
                  <button
              ...
              <>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">
                    {translate("aiflow.admin.keyName")}
                  </label>
                  <input
                    type="text"
                    value={keyName}
