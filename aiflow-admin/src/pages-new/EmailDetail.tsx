...
export function EmailDetail() {
  ...
  const { data, loading, error } = useApi<EmailDetail>(
    id ? `/api/v1/emails/${id}` : null,
  );
  ...
  const receivedFmt = data.received_date
    ? new Date(data.received_date).toLocaleString()
    : "—";
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card
          title="Intent"
          headerExtra={<IntentClassBadge value={data.intent_class} />}
        >
          <KV label="Label" value={String(intent.intent_id ?? "—")} />
          <KV
            label="Display"
            value={String(intent.intent_display_name ?? "—")}
          />
          <KV
            label="Confidence"
            value={
        ...
        <Card title="Priority">
          <KV
            label="Level"
            value={
              priority.priority_level ? `P${priority.priority_level}` : "—"
            }
          />
        ...
        <Card title="Routing">
          <KV
            label="Department"
            value={String(routing.department_name ?? "—")}
          />
          <KV label="Queue" value={String(routing.queue_name ?? "—")} />
          <KV
            label="Escalation"
            value={
              routing.escalation_triggered
                ? `yes (${String(routing.escalation_reason ?? "")})`
                : "no"
            }
          />
        </Card>
        <Card title="Meta">
          ...
          <KV
            label="Processing"
            value={`${Math.round(data.processing_time_ms)}ms`}
          />
