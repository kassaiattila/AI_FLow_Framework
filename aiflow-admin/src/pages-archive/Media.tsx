...
interface TranscriptStructured {
  title: string;
  summary: string;
...
interface MediaJob {
  ...
  transcript_structured: TranscriptStructured | null;
  error: string | null;
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              <svg
                className={`h-3 w-3 shrink-0 text-gray-400 transition-transform ${openIdx === i ? "rotate-90" : ""}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
            ...
            <span className="text-xs text-gray-400">
              ...
              {String(Math.floor(s.start_time % 60)).padStart(2, "0")} —{" "}
              {Math.floor(s.end_time / 60)}:
              {String(Math.floor(s.end_time % 60)).padStart(2, "0")}
            </span>
          </button>
          {openIdx === i && (
...
export function Media() {
  ...
  const { data, loading, error, refetch } =
    useApi<MediaResponse>("/api/v1/media");
          ...
          idx === i
            ? {
                ...
                steps: fp.steps.map((s, si) =>
                  si === 0 ? { ...s, status: "running" as const } : s,
                ),
              }
            : fp,
        ),
  ...
  const columns: Column<Record<string, unknown>>[] = [
    {
      key: "filename",
      label: translate("aiflow.media.filename"),
      render: (item) => (
    ...
    {
      key: "status",
      label: translate("aiflow.media.status"),
      render: (item) => {
    ...
    {
      key: "provider",
      label: translate("aiflow.media.provider"),
      render: (item) => (
    ...
    },
    {
      ...
      label: translate("aiflow.media.duration"),
      getValue: (item) => (item.duration_seconds as number) ?? 0,
      render: (item) => {
        const d = item.duration_seconds as number | null;
        return (
          <span className="text-gray-500 dark:text-gray-400">
            {d
  ...
  return (
    <PageLayout
      titleKey="aiflow.media.title"
      subtitleKey="aiflow.media.subtitle"
      source={data?.source}
    >
      {/* Dropzone */}
      <div
      ...
      >
        ...
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
          {translate("aiflow.media.dropzone")}
        </p>
            ...
            <div className="flex gap-2">
              ...
              >
                {uploading
                  ...
                  : translate("aiflow.media.process")}
              </button>
            </div>
          </div>
          {files.map((f, i) => (
        ...
        <div className="mb-4 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                {selectedJob.duration_seconds && (
                  <span>
                    {Math.floor(selectedJob.duration_seconds / 60)}:
                    {String(
                      Math.floor(selectedJob.duration_seconds % 60),
                    ).padStart(2, "0")}
                  </span>
                ...
                {selectedJob.processing_time_ms && (
                  <span>
                    {(selectedJob.processing_time_ms / 1000).toFixed(1)}s
                    processing
                  </span>
                )}
                {selectedJob.status === "failed" && selectedJob.error && (
                  <span className="text-red-500">
                    {selectedJob.error.substring(0, 100)}
                  </span>
                )}
              </div>
            </div>
          ...
          {selectedJob.transcript_structured ? (
            <div className="space-y-3">
              ...
              <div>
                <h4 className="text-base font-semibold text-gray-900 dark:text-gray-100">
                  {selectedJob.transcript_structured.title}
                </h4>
              ...
              {/* Key topics */}
              {selectedJob.transcript_structured.key_topics &&
                selectedJob.transcript_structured.key_topics.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {selectedJob.transcript_structured.key_topics.map(
                      (t, i) => (
                        <span
                          key={i}
              ...
              {selectedJob.transcript_structured.sections &&
                selectedJob.transcript_structured.sections.length > 0 && (
                  <TranscriptSections
                    sections={selectedJob.transcript_structured.sections}
                  />
                )}
            </div>
            ...
            <p className="text-sm text-gray-400">
              {translate("aiflow.media.noTranscript")}
            </p>
          )}
        </div>
      )}
