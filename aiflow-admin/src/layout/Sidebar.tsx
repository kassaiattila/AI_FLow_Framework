...
const MENU_GROUPS: MenuGroup[] = [
  {
    ...
    items: [
      {
        path: "/documents",
        labelKey: "aiflow.menu.documents",
        icon: "file-text",
      },
      { path: "/emails", labelKey: "aiflow.menu.emailScan", icon: "mail" },
      {
        path: "/emails/connectors",
        labelKey: "aiflow.menu.emailConnectors",
        icon: "link",
      },
      {
        path: "/emails/intent-rules",
        labelKey: "aiflow.menu.intentRules",
        icon: "filter",
      },
      {
        path: "/routing-runs",
  ...
  },
  {
    ...
    items: [
      {
        path: "/runs",
        labelKey: "aiflow.menu.pipelineRuns",
        icon: "play-circle",
      },
      {
        path: "/pipelines",
        labelKey: "aiflow.menu.pipelineTemplates",
        icon: "layers",
      },
      {
        path: "/services",
        labelKey: "aiflow.menu.serviceCatalog",
        icon: "server",
      },
    ],
  },
  {
    ...
    items: [
      { path: "/costs", labelKey: "aiflow.menu.costs", icon: "trending-up" },
      {
        ...
        labelKey: "aiflow.menu.budgets",
        icon: "bar-chart",
      },
      {
        path: "/monitoring",
        labelKey: "aiflow.menu.serviceHealth",
        icon: "activity",
      },
      {
        path: "/quality",
        labelKey: "aiflow.menu.llmQuality",
        icon: "bar-chart",
      },
    ],
  },
  {
    ...
    items: [
      ...
      { path: "/prompts", labelKey: "aiflow.menu.prompts", icon: "file-text" },
      {
        ...
        labelKey: "aiflow.menu.promptWorkflows",
        icon: "git-branch",
      },
    ],
  },
  {
    ...
    items: [
      {
        path: "/process-docs",
        labelKey: "aiflow.menu.diagrams",
        icon: "git-branch",
      },
      {
        path: "/spec-writer",
        labelKey: "aiflow.menu.specWriter",
        icon: "file-plus",
      },
      {
        path: "/media",
        labelKey: "aiflow.menu.mediaProcessing",
        icon: "headphones",
      },
...
function MenuIcon({ name }: { name: string }) {
  const icons: Record<string, string> = {
    ...
    headphones:
      "M3 18v-6a9 9 0 0118 0v6 M21 19a2 2 0 01-2 2h-1a2 2 0 01-2-2v-3a2 2 0 012-2h3z M3 19a2 2 0 002 2h1a2 2 0 002-2v-3a2 2 0 00-2-2H3z",
    "play-circle":
...
export function Sidebar() {
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>(() => {
    ...
    return Object.fromEntries(
      MENU_GROUPS.map((g) => [g.labelKey, g.defaultOpen]),
    );
  });
  ...
  return (
    <aside
      ...
      className={`flex h-full flex-col border-r border-gray-200 bg-white transition-all dark:border-gray-700 dark:bg-gray-900 ${collapsed ? "w-14" : "w-[var(--sidebar-width)]"}`}
    >
      {/* Dashboard link */}
      <nav className="flex-1 overflow-y-auto px-2 py-2">
        ...
        {MENU_GROUPS.map((group) => (
          <div key={group.labelKey} className="mt-3">
            ...
            >
              {!collapsed && (
                <span className="truncate">{translate(group.labelKey)}</span>
              )}
              {!collapsed && (
                <MenuIcon
                  name={
                    openGroups[group.labelKey] ? "chevronDown" : "chevronRight"
                  }
                />
              )}
            </button>
