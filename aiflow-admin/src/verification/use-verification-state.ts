...
function reducer(state: VerificationState, action: Action): VerificationState {
  ...
  switch (action.type) {
    case "LOAD_DATA": {
      ...
      return {
        ...INITIAL,
        dataPoints: d.data_points.map((dp) => ({ ...dp, current_value: dp.extracted_value, status: "auto" as const })),
    ...
    case "COMMIT_EDIT": {
      ...
      const oldVal = state.dataPoints.find((dp) => dp.id === eid)?.current_value ?? "";
      const nv = state.editBuffer;
      return {
        ...state,
        dataPoints: state.dataPoints.map((dp) =>
          dp.id === eid ? { ...dp, current_value: nv, status: nv !== orig ? "corrected" : "auto" } : dp
        ),
        editingPointId: null,
        editBuffer: "",
        auditLog: oldVal !== nv
          ? [...state.auditLog, { timestamp: ts(), action: "edit", field_id: eid, old_value: oldVal, new_value: nv }]
          : state.auditLog,
      };
    }
    case "CANCEL_EDIT":
    ...
    case "CONFIRM_POINT":
      return {
        ...state,
        dataPoints: state.dataPoints.map((dp) => (dp.id === action.payload ? { ...dp, status: "confirmed" } : dp)),
        auditLog: [...state.auditLog, { timestamp: ts(), action: "confirm", field_id: action.payload, old_value: null, new_value: null }],
      };
    case "CONFIRM_ALL":
      return {
        ...state,
        dataPoints: state.dataPoints.map((dp) => ({ ...dp, status: "confirmed" })),
        auditLog: [...state.auditLog, { timestamp: ts(), action: "confirm_all", field_id: null, old_value: null, new_value: `${state.dataPoints.length}` }],
      };
    case "RESET":
      return {
        ...state,
        dataPoints: state.dataPoints.map((dp) => ({ ...dp, current_value: state.originalValues[dp.id] ?? dp.extracted_value, status: "auto" })),
        editingPointId: null, editBuffer: "", selectedPointId: null,
        auditLog: [...state.auditLog, { timestamp: ts(), action: "reset", field_id: null, old_value: null, new_value: null }],
      };
    default:
      return state;
  }
...
export function useVerificationState() {
  ...
  const editChange = useCallback((value: string) => dispatch({ type: "EDIT_CHANGE", payload: value }), []);
  ...
  const stats = useMemo(() => {
    ...
    const corrected = state.dataPoints.filter((p) => p.status === "corrected").length;
    const confirmed = state.dataPoints.filter((p) => p.status === "confirmed").length;
