...
// Detect unprocessed invoices: no meaningful vendor name or zero total
...
const ListActions = ({ filterMode, onFilterModeChange }: { filterMode: FilterMode; onFilterModeChange: (mode: FilterMode) => void }) => {
  ...
  return (
    <TopToolbar>
      ...
      >
        <ToggleButton value="all">{translate("aiflow.invoices.filterAll")}</ToggleButton>
        <ToggleButton value="processed">{translate("aiflow.invoices.filterProcessed")}</ToggleButton>
      </ToggleButtonGroup>
      <FilterButton />
      {source && (
        <Chip
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FunctionField
          source="vendor.name"
          label={translate("aiflow.invoices.vendor")}
        ...
        <FunctionField
          ...
          render={(record: Record<string, unknown>) => {
            if (isUnprocessed(record)) {
              return <Chip label={translate("aiflow.invoices.unprocessed")} size="small" variant="outlined" />;
            }
            ...
            <Stack direction="row" spacing={0}>
              <Tooltip title={translate("aiflow.verification.verify")}>
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/invoices/${encodeURIComponent(String(record.id))}/verify`);
                  }}
                >
              ...
              <Tooltip title={translate("ra.message.details")}>
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/invoices/${encodeURIComponent(String(record.id))}/show`);
                  }}
                >
