...
interface NodeProps { span: TraceSpan; depth: number; traceStart: number; traceDuration: number; }

function SpanNode({ span, depth, traceStart, traceDuration }: NodeProps) {
  const [open, setOpen] = useState(depth < 1);
  const hasChildren = span.children.length > 0;
  const statusColor = span.status === "error"
    ? "bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400"
    : "bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400";

  const barLeftPct = traceDuration > 0 ? Math.max(0, ((span.start_ms - traceStart) / traceDuration) * 100) : 0;
  const barWidthPct = traceDuration > 0 ? Math.max(0.5, (span.duration_ms / traceDuration) * 100) : 1;
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="flex-1 truncate font-medium text-gray-900 dark:text-gray-100">{span.name}</span>
        {span.model && (
          <span className="hidden font-mono text-xs text-gray-500 md:inline">{span.model}</span>
        )}
        ...
        {span.cost_usd != null && span.cost_usd > 0 && (
          <span className="text-xs text-gray-500">${span.cost_usd.toFixed(4)}</span>
        )}
        <span className="text-xs tabular-nums text-gray-500">{span.duration_ms} ms</span>
        <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${statusColor}`}>{span.status}</span>
      </div>
      {/* Gantt bar */}
      <div className="relative h-1.5 px-2" style={{ paddingLeft: `${8 + depth * 16}px` }}>
...
export function TraceTree({ trace }: { trace: TraceData }) {
  if (!trace.root_spans.length) {
    return <p className="px-2 py-6 text-center text-sm text-gray-500">No spans recorded</p>;
  }
    ...
    <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      <div className="flex items-center justify-between border-b border-gray-100 px-3 py-2 dark:border-gray-800">
        <div className="text-xs text-gray-500">
          Trace <span className="font-mono text-gray-700 dark:text-gray-300">{trace.trace_id.slice(0, 12)}</span>
        </div>
        <div className="text-xs text-gray-500">
      ...
      {trace.root_spans.map((span) => (
        <SpanNode key={span.id} span={span} depth={0} traceStart={traceStart} traceDuration={traceDuration} />
      ))}
    </div>
  );
}
