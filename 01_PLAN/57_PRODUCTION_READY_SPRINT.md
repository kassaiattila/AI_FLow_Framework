...
| P4 | **Placeholder/Stub Audit** — Teljes codebase átvizsgálás: minden `placeholder`, `stub`, `TODO`, `Phase X` jelölésű nem működő funkció feltérképezése. Cél: (1) lista az összes stub-ról, (2) prioritizálás (melyik hoz valós értéket), (3) implementációs terv a hasznosakra. Ismert stubok: `PromptSyncer._push_to_langfuse`, `PromptManager._fetch_from_langfuse`, `PrometheusMetrics` (csak InMemory van), `SLAMonitor` (nincs valós alert). Ezek hasznos funkciók amik struktúrában léteznek de nem működnek — a felhasználó szempontjából "halott kód". | HIGH | `grep -r "placeholder\|stub\|Phase [0-9]" src/aiflow/` — szisztematikus audit, majd döntés: implementál / töröl / dokumentál. |
