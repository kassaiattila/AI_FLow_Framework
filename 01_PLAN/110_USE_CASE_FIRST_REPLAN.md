...
| **S107** *(was S105)* | **[DELIVERED]** `IntentRoutingPolicy` contract (`src/aiflow/policy/intent_routing.py`) + `scan_and_classify(routing_policy=...)` wiring + `POST /api/v1/emails/scan/{config_id}` optional `routing_policy_id` body field (loads `$AIFLOW_POLICY_DIR/intent_routing/{id}.yaml`). Langfuse prompt fetch path landed as opt-in breadcrumb (fetch-only; `schema_labels` extraction deferred to S108 — `PromptConfig` has no native labels field). | 4-way routing integration test GREEN on real Docker PG (`test_intent_routing.py` — invoice→EXTRACT / support→NOTIFY_DEPT / spam→ARCHIVE / unknown→MANUAL_REVIEW fallback), 4 `routed` structlog events, no `prompt_*` events when no PromptManager supplied. |
| **S108a** | **[DELIVERED]** `Emails.tsx` baseline hardening — cost preview modal (~$0.0008/email OpenAI gpt-4o-mini), BATCH_CAP=25 sequential batches, AbortController cancel, ETA via rolling avg, backend `list_emails` fallback from scan `output_data.label` to nested `intent.intent_id` so S106/S107 rows render. | tsc clean, 171/164/7 KPIs correct, cost modal shows ~$0.13/~2h 48m for 168 emails, Megszakitas leaves list consistent. |
| **S108b** | **[DELIVERED]** 3-way UI split — `/emails` daily inbox (no tabs), `/emails/connectors` standalone page via `EmailConnectors.tsx` wrapping exported `ConnectorsTab`, Upload modal triggered from action bar (no tab), sidebar adds "Email connectorok" link. DRY: `UploadTab`/`ConnectorsTab` exported, not duplicated. | Live-test 3 views (inbox + connectors + upload modal), 0 console errors across all. |
| **S108c** | **[DELIVERED]** Email bug-fix sprint — (1) `Levelada szkenneles` rewired from dead `invoice_finder` pipeline to new `POST /api/v1/emails/scan/{config_id}`, (2) new `/emails/:id` route + `EmailDetail.tsx` (intent/priority/routing/meta cards, body, attachments), (3) sibling asyncpg JSONB parse bug in `get_email` (string-vs-dict guard matching `list_emails`), (4) smaller i18n/field fixes. | Live-test confirmed row click → full detail, 0 console errors, escalation reason surfaced. |
| **S108d** | **[DELIVERED]** UX polish — retry button for failed rows (tracks `processIds` alongside `processProgress`), 7-category intent color palette (`intentColorClass`) applied in list, per-row blue-spinner "Processing..." pill while batch runs. | Intents now visually cluster by meaning (Informacio keres teal, Marketing violet, Belso emerald, …). |
| **S109a** | **[DELIVERED]** Intent Rules CRUD — 4 new backend endpoints (list/get/put/delete `/api/v1/emails/intent-rules[/{tenant_id}]`) with path-traversal guard + Pydantic `IntentRoutingPolicy` validation; new UI `/emails/intent-rules` + `/intent-rules/:tenantId` (URL-controlled list + YAML editor + schema cheatsheet + delete modal); sidebar "Intent szabalyok" link. Writes under `$AIFLOW_POLICY_DIR/intent_routing/{id}.yaml`. | 5/5 integration tests GREEN (`test_intent_rules.py`): full CRUD round-trip + path traversal + tenant_id mismatch + malformed YAML + invalid action enum. |
| **S109b** | **[DELIVERED]** Prompts YAML editor — `GET /api/v1/prompts/{name:path}` detail + `PUT` upsert (parse + name-match + write + PromptManager cache invalidate); frontend `/prompts/*` splat route → `PromptDetail.tsx` (YAML textarea + dirty tracking + inline 422 error). Langfuse SDK write-back intentionally deferred (YAML remains SSOT; `prompts/sync.py` CLI still drives Langfuse sync). | 5/5 integration tests GREEN (`test_prompts_edit.py`). Live-test: list 37 prompts → click → editor loads full YAML + version badge. |
| **S110** *(was S108)* | **[DELIVERED]** Golden-path E2E — `tests/e2e/test_uc3_emails_golden_path.py` covers Login → /emails (heading + action bar + 4 KPIs) → /emails/connectors → /emails/intent-rules → /prompts → drill-down to PromptDetail editor, all with 0 console errors. Python pytest-playwright; env-driven BASE_URL (5173 default) + auth (admin@aiflow.local). 3 live-test catalog entries under `tests/ui-live/`. | 4/4 Playwright E2E GREEN in 18s. PR + tag `v1.4.7` cut this session. |
