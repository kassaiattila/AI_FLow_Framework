...
def _client_and_headers() -> tuple[TestClient, dict[str, str]]:
    ...
    AuthProvider that signed it. Earlier tests in the suite may have
    rotated the singleton behind ``AuthProvider.from_env``, so we
    capture the *current* provider, patch ``from_env`` for app creation,
    then mint the token from that same instance.
    """
    ...
    client.get("/health/live")  # warmup the middleware stack
...
def test_list_filters_by_tenant_id() -> None:
    ...
    with patch("aiflow.api.v1.rag_collections._get_service", AsyncMock(return_value=fake)):
