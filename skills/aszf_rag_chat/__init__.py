"""ASZF RAG Chat skill - RAG pipeline for insurance documents.

Sprint T / S150 (S141-FU-3): when ``AIFLOW_PROMPT_WORKFLOWS__ENABLED=true``
the module-level :data:`prompt_manager` is built workflow-aware so the
``aszf_rag_chain`` descriptor can be resolved by
:class:`PromptWorkflowExecutor` for the baseline persona only. Flag-off
keeps the legacy single-prompt resolution byte-for-byte unchanged on
every persona.
"""
